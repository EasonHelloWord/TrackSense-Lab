...
export const pipelineController: LineController = {
  ...
  update(frame, p, context) {
    ...
    try {
      ...
      return { result: { motorCommand: { leftSpeed: speedLimit(speeds.leftSpeed), rightSpeed: speedLimit(speeds.rightSpeed) }, error: info.value, linePosition: info.value, controlOutput: control.correction, actionLabel: control.action, explanation: `${info.label}得到偏差 ${info.value.toFixed(2)}，再经${text(p, 'controlMethod')}与${text(p, 'motorMethod')}生成轮速。`, debugValues: { '偏差算法': info.label, '控制量算法': text(p, 'controlMethod'), '电机速度算法': text(p, 'motorMethod'), '控制量 u': control.correction.toFixed(2), ...control.debug } }, nextContext: { ...context, previousError: info.value, integral: control.integral, lastValidDirection: direction || context.lastValidDirection, lostLineFrames: 0 } };
