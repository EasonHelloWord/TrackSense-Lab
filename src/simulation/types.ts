...
export interface SimulationSettings { motorJitter:number; motorAcceleration:number; motorDeceleration:number; leftEfficiency:number; rightEfficiency:number; sensorNoise:number; sensorDelay:number; disabledSensor:number|null; sensorSpacing:number; sensorForwardOffset:number; motorSpacing:number; }
