...
export type TrackPreset='straight'|'left'|'right'|'hardLeft'|'hardRight'|'s'|'random'|'circle'|'ellipse'|'triangle'|'square'|'hexagon'|'lost'|'finish';
export interface SimulationSettings { motorJitter:number; motorAcceleration:number; motorDeceleration:number; leftEfficiency:number; rightEfficiency:number; sensorNoise:number; sensorDelay:number; disabledSensor:number|null; sensorSpacing:number; sensorForwardOffset:number; motorSpacing:number; loopRadius:number; }
