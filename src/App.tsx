...
export default function App() {
  ...
  const [running, setRunning] = useState(false), [teacher, setTeacher] = useState(true), [autoReset, setAutoReset] = useState(false), [renderVersion, forceRender] = useState(0), [initial, setInitial] = useState('center');
  ...
  const chooseAlgorithm = (key: 'errorMethod'|'controlMethod'|'motorMethod', value: string) => { setRunning(false); const nextParameters = { ...parameters, [key]: value }; engine.current.parameters = nextParameters; if (autoReset) engine.current.reset(); setParameters(nextParameters); refresh(); };
