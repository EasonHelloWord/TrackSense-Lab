...
import { isClosedTrack } from './simulation/trackGenerator';
...
export default function App() {
  ...
  useEffect(() => { const updatePosition = () => setControlsFloating((headerRef.current?.getBoundingClientRect().bottom ?? 0) <= 0); updatePosition(); window.addEventListener('scroll', updatePosition, { passive: true }); window.addEventListener('resize', updatePosition); return () => { window.removeEventListener('scroll', updatePosition); window.removeEventListener('resize', updatePosition); }; }, []);
